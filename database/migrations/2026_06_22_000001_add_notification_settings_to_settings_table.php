...
class AddNotificationSettingsToSettingsTable extends Migration
{
    public function up()
    {
        Schema::table('settings', function (Blueprint $table) {
            if (!Schema::hasColumn('settings', 'callmebot_api_key')) {
                $table->string('callmebot_api_key', 100)->nullable()->after('description');
            }
            if (!Schema::hasColumn('settings', 'admin_whatsapp_number')) {
                $table->string('admin_whatsapp_number', 20)->nullable()->default('8801816622128')->after('callmebot_api_key');
            }
            if (!Schema::hasColumn('settings', 'whatsapp_notify_order')) {
                $table->tinyInteger('whatsapp_notify_order')->default(1)->after('admin_whatsapp_number');
            }
            if (!Schema::hasColumn('settings', 'whatsapp_notify_member')) {
                $table->tinyInteger('whatsapp_notify_member')->default(1)->after('whatsapp_notify_order');
            }
            if (!Schema::hasColumn('settings', 'sms_notify_enabled')) {
                $table->tinyInteger('sms_notify_enabled')->default(1)->after('whatsapp_notify_member');
            }
        });
    }
