...
class DropshipperController extends Controller
{
    ...
    {
        $pageTitle = 'Dropshipper List';
        $vendors = User::where('usertype', 'dropshipper')->orderBy('id','DESC');
        ...
        $vendors = $vendors->get();
        return view('backend.dropshipper.view-dropshipper', compact('vendors', 'pageTitle'));
    }
    public function dropshipperEdit($id){
        $editData = User::find($id);
        $createDate = date('d-m-Y', $editData->subscription_plan);
        return view('backend.dropshipper.add-vendor', compact('editData','createDate'));
    }
    public function dropshipperUpdate(Request $request,$id){
        ...
        // Send welcome SMS when dropshipper is activated for the first time
        if (!$wasActive && $newStatus === 2 && !empty($plainPassword)) {
            try {
                $expireDate = $data->subscription_plan ? date('Y-m-d', $data->subscription_plan) : 'Lifetime';
                $this->sendDropshipperRegistrationSms($data, $plainPassword, 'N/A', $expireDate);
            } catch (\Exception $e) {
                Log::error('Dropshipper activation SMS failed', ['dropshipper_id' => $id, 'error' => $e->getMessage()]);
            }
        }
        
        return redirect()->back()->with('success','Dropshipper Profile update successfully !!!');
    }
    public function dropshipperList(Request $request)
    {
    ...
    public function draftView()
    {
        ...
        return view('backend.dropshipper.draft-vendor', $data);
    }
    ...
    public function dropshipperPaymentStatus($id) {
        $user = User::find($id);
        if (!$user) {
   ...
   public function delete($id)
    ...
    public function dropshipperProfileVerify($userid){
        $profiles  = \App\Models\ProfileVerify::where('user_id',$userid)->first();
        return view('backend.dropshipper.dropshipper-profile-verify',compact('profiles'));
    }
    public function dropshipperProfileDelete($userid){
        $profiles  = \App\Models\ProfileVerify::where('user_id',$userid)->first();
    ...
    public function dropshipperStatus($id, $status){
    ...
    public function dropshipperPaymentStatus_new($id, $payment_status){
        $user = User::findOrFail($id);
        if($payment_status == 0){
   ...
   public function dropshipperCommission(Request $request)
{
